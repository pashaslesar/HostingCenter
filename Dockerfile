...
RUN apk update && apk add \
    ...
    php-pdo php-pdo_mysql nodejs npm db-utils  # ← добавили db-utils
...
RUN apk update && apk add --no-cache \
    ...
    postgresql openrc db-utils  # ← добавили db-utils
...
# Копируем скрипт и делаем исполняемым
COPY ./.docker/vsftpd/scripts/setup_ftp_user.sh /usr/local/bin/setup_ftp_user.sh
RUN chmod +x /usr/local/bin/setup_ftp_user.sh

RUN chmod +x /lib/security/pam_pwdfile.so

# Устанавливаем dos2unix и конвертируем конфиги
RUN apk add dos2unix
RUN find /etc/vsftpd -type f -exec dos2unix {} \;
